关于AI Security Team
    构建ai-security-team初心，利用AI将现有的运维体系进行重新整合，更好的面对未来的未知风险。ai-security-team在构建初期将核心的运维工作都进行了设计，但这一类工程的最大特点就是它会存在很多定制化，每个人每个团队每个公司都会有自己的ai-security-team，我提供的这个更多的只是给大家带来一个新的思考方向，希望别被我的框架所局限。我们需要审视自己所处的环境来进行优化而不是一味的进行使用，AI领域里不管是大模型或者应用都是需要优化的，标品化只会导致AI效率低下。
    团队中还有很多需要去解决并且做出定义的内容，这些都可以依靠我们去引导AI完成，不建议手动强行介入，不要用自己的能力边界去限制AI的边界，把他当作一个合作伙伴一起讨论是最好的方式。
